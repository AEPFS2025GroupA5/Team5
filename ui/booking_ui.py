...
def get_userfriendly_room(room: model.Room) -> str:
    ...
    return f"Hotel: {h.name}, Stars: {h.stars}, {h.address.city}, Room number: {r.room_number}, Room description: {r.room_type.description}, Max Guests: {r.room_type.max_guests}, Price per night: CHF {r.price_per_night}"

def get_userfriendly_price(room: model.Room) -> str:
    r = roomman.get_room_by_id(room.room_id)
    h = hotelman.read_hotel_by_id(r.hotel.hotel_id)
    return f"Hotel: {h.name}, Stars: {h.stars}, City: {h.address.city}, Room number: {r.room_number}, Room description: {r.room_type.description}, Max Guests: {r.room_type.max_guests}"
